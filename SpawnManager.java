...
import com.survivor.util.MapWall;
...
//        System.out.println("⚠ 无法在允许范围内找到合适刷怪点");
    ...
    {
        try {
            // 使用 Files.lines() 逐行读取文件，返回一个 Stream
            Files.lines(Paths.get("walls.txt"))
                    .map(line -> line.trim().split("\\s+")) // 按空格分割每行
                    .filter(parts -> parts.length == 4)    // 过滤掉不符合格式的行
                    .forEach(parts -> {
                        // 将字符串转换为整数
                        ...
                        // 在这里直接使用 FXGL.spawn() 生成实体
                        // 假设你有一个名为 "myEntity" 的工厂方法，它接受 w, h, x, y
//                        FXGL.spawn("wall", new SpawnData(x, y).put("w", w).put("h", h).put("startPos",new Point2D(x,y)));
                        mapWall.spawn();
                        walls.add(mapWall);
